...
public class CombatLogEntryService(IRepository<CombatLogEntry> repository) : ICrudService<CombatLogEntry>
{
    ...
    public Task<CombatLogEntry> CreateAsync(CombatLogEntry entity, CancellationToken ct = default) => repository.AddAsync(entity, true, ct);
    public Task UpdateAsync(CombatLogEntry entity, CancellationToken ct = default) => repository.UpdateAsync(entity, true, ct);
    public Task DeleteAsync(CombatLogEntry entity, CancellationToken ct = default) => repository.DeleteAsync(entity, true, ct);
}
