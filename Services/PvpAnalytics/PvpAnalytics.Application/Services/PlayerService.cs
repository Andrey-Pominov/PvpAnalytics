...
public class PlayerService(IRepository<Player> repository) : ICrudService<Player>
{
    ...
    public Task<Player> CreateAsync(Player entity, CancellationToken ct = default) => repository.AddAsync(entity, ct);
    public Task UpdateAsync(Player entity, CancellationToken ct = default) => repository.UpdateAsync(entity, ct);
    public Task DeleteAsync(Player entity, CancellationToken ct = default) => repository.DeleteAsync(entity, ct);
}
