...
public class MatchService(IRepository<Match> repository) : ICrudService<Match>
{
    ...
    public Task<Match> CreateAsync(Match entity, CancellationToken ct = default) => repository.AddAsync(entity, true, ct);
    public Task UpdateAsync(Match entity, CancellationToken ct = default) => repository.UpdateAsync(entity,true,  ct);
    public Task DeleteAsync(Match entity, CancellationToken ct = default) => repository.DeleteAsync(entity, true, ct);
}
