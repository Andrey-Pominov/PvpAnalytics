...
public class PaymentService(IRepository<Payment> repository) : ICrudService<Payment>
{
    ...
    public Task UpdateAsync(Payment entity, CancellationToken ct = default) => repository.UpdateAsync(entity, ct);
    public Task DeleteAsync(Payment entity, CancellationToken ct = default) => repository.DeleteAsync(entity, ct);
}
